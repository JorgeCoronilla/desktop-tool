...
const xlsx_calc_1 = __importDefault(require("xlsx-calc"));
...
class ElectronMCPServer {
            ...
            },
            {
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        maxChunkSize: {
                            type: 'number',
                            description: 'Tamaño máximo del fragmento en caracteres (opcional, default: 8000)',
                        },
                    },
                    ...
                    properties: {
                        ...
                        modifications: {
                            ...
                            properties: {
                                sheetName: {
                                    type: 'string',
                                    description: 'Nombre de la hoja (opcional, por defecto la primera hoja)',
                                },
                                addRows: {
                                    type: 'array',
                                    items: {
                                ...
                                deleteRows: {
                                    type: 'array',
                                    items: { type: 'number' },
                                    description: 'Índices de filas a eliminar (empezando desde 0)',
                                },
                            },
                        },
                    },
            ...
            },
            {
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        sheetName: {
                            type: 'string',
                            description: 'Nombre de la hoja específica (opcional, por defecto lee la primera hoja)',
                        },
                        calculateFormulas: {
                            type: 'boolean',
                            description: 'Si calcular las fórmulas automáticamente antes de leer (por defecto: true)',
                        },
                    },
                    required: ['filePath'],
                },
                    ...
                    properties: {
                        ...
                        formulas: {
                            ...
                            properties: {
                                sheetName: {
                                    type: 'string',
                                    description: 'Nombre de la hoja donde agregar las fórmulas (opcional, por defecto la primera hoja)',
                                },
                                cellFormulas: {
                                    type: 'array',
                                    items: {
                                        ...
                                        properties: {
                                            cell: {
                                                type: 'string',
                                                description: 'Dirección de la celda donde colocar la fórmula (ejemplos: A1, B2, C10)',
                                            },
                                            formula: {
                                                type: 'string',
                                                description: 'Fórmula de Excel a agregar (ejemplos: =SUM(A1:A10), =AVERAGE(B1:B5), =A1*B1)',
                                            },
                                        },
            ...
            },
            {
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        sheetName: {
                            type: 'string',
                            description: 'Nombre de la hoja específica (opcional, por defecto calcula todas las hojas)',
                        },
                    },
                    required: ['filePath'],
                },
            },
            {
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        sheetName: {
                            type: 'string',
                            description: 'Nombre de la hoja específica (opcional, por defecto analiza la primera hoja)',
                        },
                    },
                    required: ['filePath'],
                },
    ...
    async deleteFileOrDirectory(args) {
    ...
    async deleteMultipleItems(args) {
        const results = [];
    ...
    async ocrPdf(args) {
        ...
        try {
            ...
            const convert = (0, pdf2pic_1.fromPath)(resolvedPath, {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout en conversión PDF a imagen (30s)')), 30000));
            ...
            try {
                ...
                const ocrTimeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout en OCR (60s)')), 60000));
                const { data: { text }, } = (await Promise.race([ocrPromise, ocrTimeoutPromise]));
        ...
        try {
            ...
            switch (strategy) {
                ...
                case 'preview':
                    ...
                    metadata.ocrUsed = true;
                    return await this.readPdfSmartWithOcr(resolvedPath, {
                        ...args,
                        strategy,
                        maxPages,
                        maxCharacters,
                        useOcr,
                        pages: Array.from({ length: Math.min(maxPages, metadata.totalPages) }, (_, i) => i + 1),
                    }, metadata);
                case 'pages':
    ...
    async readPdfSmartWithOcr(resolvedPath, args, metadata) {
        ...
        try {
            ...
            for (const pageNum of pagesToProcess) {
                try {
                    ...
                    const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`Timeout en conversión de página ${pageNum}`)), 30000));
                    const result = await Promise.race([
                        conversionPromise,
                        timeoutPromise,
                    ]);
                    ...
                    try {
                        ...
                        const ocrTimeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`Timeout en OCR de página ${pageNum}`)), 60000));
    ...
    async modifyExcel(args) {
        ...
        if (args.modifications.deleteRows) {
            const sortedIndices = args.modifications.deleteRows.sort((a, b) => b - a);
    ...
    async readExcelWithFormulas(args) {
        // Resolver ruta relativa si se proporciona cwd
        let resolvedPath = args.filePath;
        if (args._options?.cwd && !path.isAbsolute(args.filePath)) {
            resolvedPath = path.resolve(args._options.cwd, args.filePath);
        ...
        if (args.calculateFormulas !== false) {
            ...
            try {
                (0, xlsx_calc_1.default)(workbook);
            }
            catch (calcError) {
    ...
    async addExcelFormulas(args) {
        ...
        const workbook = XLSX.readFile(resolvedPath);
        const targetSheetName = args.formulas.sheetName &&
        ...
        for (const cellFormula of args.formulas.cellFormulas) {
            ...
            if (cellRef.r > range.e.r)
                range.e.r = cellRef.r;
            if (cellRef.c > range.e.c)
                range.e.c = cellRef.c;
            if (cellRef.r < range.s.r)
                range.s.r = cellRef.r;
            if (cellRef.c < range.s.c)
                range.s.c = cellRef.c;
        }
    ...
    async calculateExcelFormulas(args) {
        // Resolver ruta relativa si se proporciona cwd
        let resolvedPath = args.filePath;
        if (args._options?.cwd && !path.isAbsolute(args.filePath)) {
            resolvedPath = path.resolve(args._options.cwd, args.filePath);
    ...
    async getExcelFormulasInfo(args) {
        // Resolver ruta relativa si se proporciona cwd
        let resolvedPath = args.filePath;
        if (args._options?.cwd && !path.isAbsolute(args.filePath)) {
            resolvedPath = path.resolve(args._options.cwd, args.filePath);
