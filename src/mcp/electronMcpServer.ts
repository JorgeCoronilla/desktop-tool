...
// Importar el registro de herramientas para el sistema automático
...
export class ElectronMCPServer {
  ...
  private useAutomaticRegistration: boolean;

  constructor(options: { useAutomaticRegistration?: boolean } = {}) {
    this.tools = new Map();
    this.toolRegistry = new ToolRegistry();
    this.useAutomaticRegistration = options.useAutomaticRegistration ?? false;
    
    this.initializeTools();
  }

  private async initializeTools() {
    if (this.useAutomaticRegistration) {
      await this.initializeAutomaticRegistration();
    } else {
      this.setupTools();
    }
  }
  ...
  private setupTools() {
    ...
    this.registerTool(
      ...
      this.deleteFileOrDirectory.bind(this)
    );
    this.registerTool('list_directory', this.listDirectory.bind(this));
    this.registerTool(
      'copy_file_or_directory',
      this.copyFileOrDirectory.bind(this)
    );
    this.registerTool(
      'move_file_or_directory',
      this.moveFileOrDirectory.bind(this)
    );
    this.registerTool(
      'list_files_by_criteria',
      this.listFilesByCriteria.bind(this)
    );
    this.registerTool(
    ...
    this.registerTool('style_excel_cells', this.styleExcelCells.bind(this));

    // Herramientas de Excel con fórmulas
    this.registerTool(
      'read_excel_with_formulas',
      this.readExcelWithFormulas.bind(this)
    );
    this.registerTool('add_excel_formulas', this.addExcelFormulas.bind(this));
    this.registerTool(
      'calculate_excel_formulas',
      this.calculateExcelFormulas.bind(this)
    );
    this.registerTool(
      'get_excel_formulas_info',
      this.getExcelFormulasInfo.bind(this)
    );
  }
  ...
  private async initializeAutomaticRegistration() {
    try {
      console.log('🔄 Inicializando sistema de registro automático de herramientas...');
      ...
      console.log('🔄 Usando sistema manual como fallback...');
      this.setupTools();
    }
  }
  ...
  async executeTool(
    ...
    try {
      if (this.useAutomaticRegistration) {
        // Usar el sistema de registro automático
        ...
        // Usar el sistema manual tradicional
  ...
  getOpenAIFunctions() {
    ...
    // Sistema manual tradicional - convertir herramientas a formato OpenAI
  ...
  /**
   * Obtiene las herramientas manuales (método auxiliar)
   */
  private getManualTools() {
    return [
      {
        name: 'read_text_file',
        ...
        inputSchema: {
          ...
          properties: {
            paths: {
              ...
              items: { type: 'string' },
              description:
                'Array de rutas de archivos o directorios a eliminar',
            },
          },
          required: ['paths'],
        },
      },
      {
        name: 'read_pdf_smart',
        description:
          'USAR SIEMPRE PRIMERO: Herramienta principal para leer PDFs con estrategias inteligentes. Automáticamente elige la mejor estrategia (lectura directa o OCR) y permite optimizar rendimiento. Use "preview" para facturas/documentos cortos.',
        inputSchema: {
          type: 'object',
          properties: {
            filePath: {
            ...
            strategy: {
              ...
              description:
                'Estrategia: "preview" (facturas/docs cortos, default), "full" (documento completo), "pages" (páginas específicas), "limit" (límite caracteres)',
              default: 'preview',
            },
            pages: {
              type: 'array',
              items: { type: 'number' },
              description:
                'Páginas específicas a leer (solo con strategy="pages"). Ej: [1, 3, 5]',
            },
            maxPages: {
              type: 'number',
              description:
                'Máximo páginas desde el inicio (strategy="preview"). Default: 3',
              default: 3,
            },
            maxCharacters: {
              type: 'number',
              description:
                'Límite de caracteres (strategy="limit"). Default: 10000',
              default: 10000,
            },
            useOcr: {
              type: 'boolean',
              description:
                'Forzar OCR aunque haya texto directo. Default: false',
              default: false,
            },
          },
          required: ['filePath'],
      ...
      },
      {
        name: 'read_pdf',
        description:
          'HERRAMIENTA LEGACY: Lee texto básico de PDFs. Solo usar si read_pdf_smart no está disponible o falla.',
        inputSchema: {
          type: 'object',
          properties: {
            filePath: {
      ...
      },
      {
        name: 'read_pdf_chunk',
        description:
          'HERRAMIENTA LEGACY: Lee fragmentos de PDFs grandes. Solo usar si read_pdf_smart no maneja PDFs grandes correctamente.',
        inputSchema: {
          type: 'object',
          properties: {
            ...
            chunkIndex: {
              type: 'number',
              description: 'Índice del fragmento a leer (0-based)',
            },
            maxChunkSize: {
              type: 'number',
              description:
                'Tamaño máximo del fragmento en caracteres (opcional, default: 8000)',
            },
          },
      ...
      },
      {
        name: 'ocr_pdf',
        description:
          'HERRAMIENTA LEGACY: OCR directo para PDFs escaneados. Solo usar si read_pdf_smart con useOcr=true no funciona.',
        inputSchema: {
          type: 'object',
          properties: {
            filePath: {
              type: 'string',
              description: 'Ruta del archivo PDF escaneado para OCR',
            },
          },
          required: ['filePath'],
        },
      ...
      },
      {
        name: 'write_excel',
        description:
          'Crea un archivo Excel con datos. IMPORTANTE: Siempre debes proporcionar el parámetro "data" con un array de objetos que representen las filas. Ejemplo: [{"Provincia": "Madrid", "Capital": "Madrid"}, {"Provincia": "Barcelona", "Capital": "Barcelona"}]',
        inputSchema: {
          type: 'object',
          properties: {
            filePath: {
            ...
            data: {
              ...
              description:
                'REQUERIDO: Array de objetos donde cada objeto representa una fila. Las claves del objeto serán las columnas. Ejemplo: [{"Nombre": "Juan", "Edad": 30}, {"Nombre": "Ana", "Edad": 25}]',
            },
            sheetName: {
              type: 'string',
      ...
      },
      {
        name: 'modify_excel',
        description:
          'Modifica un archivo Excel existente agregando, actualizando o eliminando filas. Ejemplo: para agregar filas usa addRows: [{"Nombre": "Juan", "Edad": 30}]',
        inputSchema: {
          type: 'object',
          properties: {
            filePath: {
            ...
            modifications: {
              ...
              properties: {
                sheetName: {
                  type: 'string',
                  description:
                    'Nombre de la hoja (opcional, por defecto la primera hoja)',
                },
                addRows: {
                  type: 'array',
                  items: {
                  ...
                  description:
                    'Filas a agregar como array de objetos. Ejemplo: [{"Columna1": "valor1", "Columna2": "valor2"}]',
                },
                updateRows: {
                  type: 'array',
                  items: {
                  ...
                  description:
                    'Filas a actualizar como array de objetos con índice. Ejemplo: [{"index": 0, "Columna1": "nuevo_valor"}]',
                },
                deleteRows: {
                  type: 'array',
                  items: { type: 'number' },
                  description:
                    'Índices de filas a eliminar (empezando desde 0)',
                },
              },
            },
          },
      ...
      },
      {
        ...
        description:
          'Aplica estilos a celdas o rangos de celdas en un archivo de Excel existente.',
        inputSchema: {
          type: 'object',
          properties: {
            filePath: {
            ...
            styles: {
              type: 'array',
              description: 'Estilos a aplicar en el archivo',
              items: {
                type: 'object',
                properties: {
                  range: {
                    type: 'string',
                    description: 'Rango de celdas (ej. \"A1\", \"A1:B10\")',
                  },
                  style: {
                    type: 'object',
                    description: 'Objeto de estilo a aplicar',
                  },
                  sheetName: {
                    type: 'string',
                    description:
      ...
      },
      {
        ...
        description:
          'Lee un archivo Excel con soporte completo para fórmulas. Calcula automáticamente las fórmulas y extrae información detallada. Útil para archivos con cálculos complejos.',
        inputSchema: {
          type: 'object',
          properties: {
            filePath: {
            ...
            sheetName: {
              type: 'string',
              description:
                'Nombre de la hoja específica (opcional, por defecto lee la primera hoja)',
            },
            calculateFormulas: {
              type: 'boolean',
              description:
                'Si calcular las fórmulas automáticamente antes de leer (por defecto: true)',
            },
          },
          required: ['filePath'],
        },
      },
      {
        ...
        description:
          'Agrega fórmulas a celdas específicas en un archivo Excel existente. Ejemplo: agregar =SUM(A1:A10) en la celda B1',
        inputSchema: {
          type: 'object',
          properties: {
            filePath: {
              type: 'string',
              description: 'Ruta del archivo Excel existente',
            },
            formulas: {
              type: 'object',
              ...
              properties: {
                sheetName: {
                  type: 'string',
                  description:
                    'Nombre de la hoja donde agregar las fórmulas (opcional, por defecto la primera hoja)',
                },
                cellFormulas: {
                  type: 'array',
                  items: {
                    ...
                    properties: {
                      cell: {
                        type: 'string',
                        description:
                          'Dirección de la celda donde colocar la fórmula (ejemplos: A1, B2, C10)',
                      },
                      formula: {
                        type: 'string',
                        description:
                          'Fórmula de Excel a agregar (ejemplos: =SUM(A1:A10), =AVERAGE(B1:B5), =A1*B1)',
                      },
                    },
                    required: ['cell', 'formula'],
                  },
                  description:
                    'Array de fórmulas a agregar. Ejemplo: [{"cell": "C1", "formula": "=A1+B1"}]',
                },
              },
      ...
      },
      {
        ...
        description:
          'Calcula todas las fórmulas en una hoja de Excel y guarda los resultados calculados en el archivo. Útil para actualizar valores después de cambios en los datos.',
        inputSchema: {
          type: 'object',
          properties: {
            filePath: {
              type: 'string',
              description: 'Ruta del archivo Excel con fórmulas a calcular',
            },
            sheetName: {
              type: 'string',
              description:
                'Nombre de la hoja específica (opcional, por defecto calcula todas las hojas)',
            },
          },
          required: ['filePath'],
        },
      },
      {
        ...
        description:
          'Obtiene información detallada sobre todas las fórmulas en una hoja de Excel, incluyendo ubicación, dependencias, tipos de fórmulas y estadísticas. No modifica el archivo.',
        inputSchema: {
          type: 'object',
          properties: {
            filePath: {
            ...
            sheetName: {
              type: 'string',
              description:
                'Nombre de la hoja específica (opcional, por defecto analiza la primera hoja)',
            },
          },
          required: ['filePath'],
        },
  ...
  private async readTextFile(args: {
    filePath: string;
    _options?: { cwd?: string };
  }) {
  ...
  private async writeTextFile(args: {
    filePath: string;
    content: string;
    _options?: { cwd?: string };
  }) {
  ...
  private async createDirectory(args: {
    dirPath: string;
    _options?: { cwd?: string };
  }) {
  ...
  private async deleteFileOrDirectory(args: {
    targetPath: string;
    _options?: { cwd?: string };
  }) {
  ...
  private async copyFileOrDirectory(args: {
    sourcePath: string;
    destPath: string;
  }) {
  ...
  private async moveFileOrDirectory(args: {
    sourcePath: string;
    destPath: string;
  }) {
  ...
  private async listFilesByCriteria(args: {
    dirPath: string;
    extension?: string;
    namePattern?: string;
    recursive?: boolean;
    ...
    try {
      ...
      if (!data.text || data.text.trim().length === 0) {
        ...
        try {
          ...
          throw new Error(
            `No se pudo extraer texto del PDF ni mediante OCR: ${ocrError instanceof Error ? ocrError.message : 'Error desconocido'}`
          );
        }
      }
      ...
      throw new Error(
        `Error al leer PDF: ${errorMessage}. Intenta usar ocr_pdf para PDFs escaneados.`
      );
    }
  }

  private async readPdfChunk(args: {
    filePath: string;
    chunkIndex: number;
    maxChunkSize?: number;
    _options?: { cwd?: string };
    ...
    try {
      ...
      if (chunkIndex < 0 || chunkIndex >= chunks.length) {
        throw new Error(
          `Índice de chunk inválido: ${chunkIndex}. Chunks disponibles: 0-${chunks.length - 1}`
        );
      }
  ...
  private async ocrPdf(args: {
    ...
    try {
      ...
      throw new Error(
        `Error al validar PDF: ${error instanceof Error ? error.message : 'Error desconocido'}`
      );
    }
    ...
    try {
      ...
      const timeoutPromise = new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error('Timeout en conversión PDF a imagen (30s)')),
          30000
        )
      );
      ...
      try {
        ...
        const {
          data: { text },
        } = (await Promise.race([ocrPromise, ocrTimeoutPromise])) as any;
        console.log(
          '[OCR] OCR completado, texto extraído:',
          text.length,
          'caracteres'
        );
      ...
      throw new Error(
        `No se pudo procesar el PDF con OCR: ${ocrError instanceof Error ? ocrError.message : 'Error desconocido'}`
      );
    } finally {
  ...
  private async readPdfSmart(args: {
    ...
    switch (strategy) {
      ...
      case 'pages':
        console.log(
          `📑 [PDF_SMART] Páginas específicas: [${args.pages?.join(', ') || 'No especificadas'}]`
        );
        break;
      case 'limit':
    ...
    try {
      ...
      if (useOcr) {
        console.log(
          `🔧 [PDF_SMART] DECISIÓN: Forzando uso de OCR por parámetro useOcr=true`
        );
        return await this.readPdfSmartWithOcr(
          resolvedPath,
          { ...args, strategy, maxPages, maxCharacters, useOcr },
          metadata
        );
      }
      ...
      console.log(
        `📖 [PDF_SMART] DECISIÓN: Intentando lectura directa del PDF primero`
      );
      ...
      console.log(
        `📊 [PDF_SMART] PDF analizado: ${metadata.totalPages} páginas, ${pdfData.text?.length || 0} caracteres de texto directo`
      );

      // Verificar si hay texto extraíble
      if (!pdfData.text || pdfData.text.trim().length < 50) {
        console.log(
          `⚠️  [PDF_SMART] DECISIÓN: Texto insuficiente en lectura directa (${pdfData.text?.trim().length || 0} chars), cambiando a OCR automáticamente`
        );
      ...
      console.log(
        `✅ [PDF_SMART] DECISIÓN: Texto directo suficiente, aplicando estrategia ${strategy.toUpperCase()}`
      );
      ...
      switch (strategy) {
        case 'full':
          console.log(
            `📚 [PDF_SMART] PROCESANDO: Estrategia FULL - Extrayendo todo el texto (${pdfData.text.length} caracteres)`
          );
          extractedText = pdfData.text;
          metadata.pagesRead = Array.from(
        ...
        case 'preview':
          ...
          console.log(
            `📖 [PDF_SMART] DECISIÓN: Estrategia PREVIEW requiere OCR para páginas específicas (1-${Math.min(maxPages, metadata.totalPages)})`
          );
          metadata.ocrUsed = true;
          return await this.readPdfSmartWithOcr(
            resolvedPath,
            {
              ...
              useOcr,
              pages: Array.from(
                { length: Math.min(maxPages, metadata.totalPages) },
                (_, i) => i + 1
              ),
            },
            metadata
        ...
        case 'pages':
          if (!args.pages || args.pages.length === 0) {
            throw new Error(
              'Para strategy="pages" se requiere el parámetro pages con números de página'
            );
          }
          // Para páginas específicas, necesitamos OCR
          console.log(
            `📑 [PDF_SMART] DECISIÓN: Estrategia PAGES requiere OCR para páginas específicas [${args.pages.join(', ')}]`
          );
          metadata.ocrUsed = true;
          return await this.readPdfSmartWithOcr(
            resolvedPath,
        ...
        case 'limit':
          console.log(
            `📏 [PDF_SMART] PROCESANDO: Estrategia LIMIT - Limitando a ${maxCharacters} caracteres de ${pdfData.text.length} totales`
          );
      ...
      console.log(
        `📊 [PDF_SMART] Caracteres extraídos: ${metadata.charactersExtracted}`
      );
      console.log(
        `📄 [PDF_SMART] Páginas procesadas: [${metadata.pagesRead.join(', ')}]`
      );
      console.log(
        `🔧 [PDF_SMART] OCR utilizado: ${metadata.ocrUsed ? 'SÍ' : 'NO'}`
      );
      ...
      if (!args.useOcr) {
        console.log(
          `🔄 [PDF_SMART] FALLBACK: Cambiando a OCR debido a error en lectura directa`
        );
        console.log(
          `🎯 [PDF_SMART] FALLBACK: Estrategia ${args.strategy.toUpperCase()} será procesada con OCR`
        );
        const metadata = {
      ...
      console.error(
        `❌ [PDF_SMART] ERROR FINAL: No se pudo procesar el PDF con ningún método`
      );
      throw error;
    }
  }

  private async readPdfSmartWithOcr(
    resolvedPath: string,
    args: any,
    metadata: any
  ) {
    ...
    console.log(
      `🎯 [PDF_SMART_OCR] Estrategia: ${args.strategy.toUpperCase()}`
    );
    ...
    try {
      ...
      if (!metadata.totalPages) {
        try {
          ...
          console.log(
            `📊 [PDF_SMART_OCR] Total de páginas detectado: ${totalPages}`
          );
        } catch (pageCountError) {
          console.log(
            `⚠️  [PDF_SMART_OCR] No se pudo determinar el número de páginas, asumiendo 1`
          );
          totalPages = 1;
        }
      }
      ...
      switch (args.strategy) {
        case 'full':
          ...
          pagesToProcess = Array.from(
            { length: maxPagesForFull },
            (_, i) => i + 1
          );
          console.log(
            `📚 [PDF_SMART_OCR] FULL: Procesando hasta ${maxPagesForFull} páginas de ${totalPages} totales`
          );
          break;
        ...
        case 'preview':
          ...
          console.log(
            `📖 [PDF_SMART_OCR] PREVIEW: Procesando primeras ${maxPages} páginas de ${totalPages} totales`
          );
          break;
        ...
        case 'pages':
          ...
          pagesToProcess = requestedPages.filter(
          ...
          if (pagesToProcess.length !== requestedPages.length) {
            ...
            console.log(
              `⚠️  [PDF_SMART_OCR] PAGES: Páginas inválidas ignoradas [${invalidPages.join(', ')}] - PDF solo tiene ${totalPages} páginas`
            );
          }
          console.log(
            `📑 [PDF_SMART_OCR] PAGES: Procesando páginas válidas [${pagesToProcess.join(', ')}] de ${totalPages} totales`
          );
          break;
        ...
        case 'limit':
          ...
          console.log(
            `📏 [PDF_SMART_OCR] LIMIT: Procesando página 1 de ${totalPages} totales (límite: ${args.maxCharacters || 10000} chars)`
          );
          break;
      }
      ...
      if (pagesToProcess.length === 0) {
        throw new Error(
          `No hay páginas válidas para procesar. PDF tiene ${totalPages} páginas.`
        );
      }

      console.log(
        `🔄 [PDF_SMART_OCR] Páginas a procesar: [${pagesToProcess.join(', ')}]`
      );
      ...
      for (const pageNum of pagesToProcess) {
        try {
          ...
          const timeoutPromise = new Promise((_, reject) =>
            setTimeout(
              () =>
                reject(new Error(`Timeout en conversión de página ${pageNum}`)),
              30000
            )
          );
          ...
          if (!result || !result.path) {
            console.log(
              `[OCR] No se pudo convertir la página ${pageNum}, saltando...`
            );
            continue;
          }
          ...
          if (!fs.existsSync(result.path)) {
            console.log(
              `[OCR] Archivo de imagen no encontrado para página ${pageNum}, saltando...`
            );
            continue;
          }
          ...
          console.log(
            `[OCR] Iniciando reconocimiento OCR para página ${pageNum}...`
          );
          const createWorker = await initializeTesseract();
          ...
          try {
            ...
            const ocrTimeoutPromise = new Promise((_, reject) =>
              setTimeout(
                () => reject(new Error(`Timeout en OCR de página ${pageNum}`)),
                60000
              )
            );
            ...
            if (text && text.trim().length > 0) {
              ...
              console.log(
                `[OCR] Página ${pageNum} procesada exitosamente - ${text.length} caracteres`
              );
              ...
              if (args.strategy === 'limit') {
                ...
                if (combinedText.length >= maxChars) {
                  ...
                  console.log(
                    `[OCR] Límite de caracteres alcanzado: ${maxChars}`
                  );
                  break;
                }
              }
          ...
          try {
            ...
            console.log(
              `[OCR] Error limpiando archivo temporal: ${cleanupError}`
            );
          }
        } catch (pageError) {
      ...
      console.log(
        `📊 [PDF_SMART_OCR] Páginas procesadas exitosamente: [${processedPages.join(', ')}]`
      );
      console.log(
        `📏 [PDF_SMART_OCR] Caracteres extraídos: ${combinedText.length}`
      );
      console.log(
        `🎯 [PDF_SMART_OCR] Estrategia aplicada: ${args.strategy.toUpperCase()}`
      );
  ...
  private async writeExcel(args: {
    ...
    XLSX.utils.book_append_sheet(
      workbook,
      worksheet,
      args.sheetName || 'Hoja1'
    );
    XLSX.writeFile(workbook, resolvedPath);
    return { success: true };
  }

  private async modifyExcel(args: {
    filePath: string;
    modifications: any;
    _options?: { cwd?: string };
  }) {
    ...
    if (args.modifications.deleteRows) {
      const sortedIndices = args.modifications.deleteRows.sort(
  ...
  private async styleExcelCells(args: {
    filePath: string;
    styles: Array<{
      range: string;
      style: any;
    ...
    for (const styleInfo of args.styles) {
      const sheetName = styleInfo.sheetName || workbook.SheetNames[0];
      const worksheet = workbook.Sheets[sheetName];
      if (!worksheet) continue;

      const range = XLSX.utils.decode_range(styleInfo.range);
      for (let R = range.s.r; R <= range.e.r; ++R) {
        for (let C = range.s.c; C <= range.e.c; ++C) {
          ...
          if (!cell) {
            cell = { t: 'z', v: '' }; // Create empty cell if it doesn't exist
            worksheet[cell_ref] = cell;
          }
          cell.s = { ...cell.s, ...styleInfo.style };
        }
      }
    }
  ...
  private async readExcelWithFormulas(args: {
    ...
    const targetSheetName =
      args.sheetName && workbook.SheetNames.includes(args.sheetName)
        ? args.sheetName
    ...
    const formulaData = XLSX.utils.sheet_to_json(sheet, {
      defval: null,
      raw: true,
    });
  ...
  private async addExcelFormulas(args: {
    ...
    const targetSheetName =
      ...
      workbook.SheetNames.includes(args.formulas.sheetName)
        ? args.formulas.sheetName
        : workbook.SheetNames[0];
  ...
  private async calculateExcelFormulas(args: {
    ...
    try {
      ...
      throw new Error(
        `Error calculando fórmulas: ${calcError instanceof Error ? calcError.message : 'Error desconocido'}`
      );
    }
  ...
  private async getExcelFormulasInfo(args: {
    ...
    // Estadísticas
    const formulaTypes = formulas.reduce(
      (acc, f) => {
