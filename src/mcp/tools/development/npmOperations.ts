...
const NpmOperationsSchema = z.object({
  operation: z.enum(['install', 'uninstall', 'update', 'list', 'search', 'run', 'init', 'audit', 'outdated']).describe('Operación npm a realizar'),
  package: z.string().optional().describe('Nombre del paquete (para install, uninstall, search)'),
  script: z.string().optional().describe('Nombre del script a ejecutar (para run)'),
  workingDirectory: z.string().optional().describe('Directorio donde ejecutar npm'),
  options: z.array(z.string()).optional().describe('Opciones adicionales para el comando npm'),
  global: z.boolean().optional().describe('Instalar globalmente (solo para install)'),
  packages: z.array(z.string()).optional().describe('Lista de paquetes (para install, uninstall, update)'),
});

export const npmOperationsTool = {
  name: 'npm_operations',
    ...
    try {
      ...
      try {
        await new Promise<void>((resolve, reject) => {
          ...
          child.on('close', (code) => {
            if (code === 0) resolve();
            else reject(new Error('npm no está disponible'));
          });
